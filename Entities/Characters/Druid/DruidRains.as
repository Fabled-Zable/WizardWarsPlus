...
class DruidRain
{
    ...
    DruidRain(CBlob@ blob, u8 i_type, u8 i_level, Vec2f pos)
    {
        type = i_type;
        level = i_level;
        position = pos;
        ...
        if (type == DruidRainTypes::zombieRain)
        {
            if (level == DruidParams::extra_ready)
                SummonBlob(blob, "zombieknight", position, team);
            objectsAmount = 5;
            if (level == DruidParams::extra_ready)
                objectsAmount += XORRandom(15);
            else if (level == DruidParams::cast_3)
                objectsAmount += XORRandom(10);
            else if (level == DruidParams::cast_2)
                objectsAmount += XORRandom(6);
            else if (level == DruidParams::cast_1)
                objectsAmount += XORRandom(3);
            time = 1 + XORRandom(6);
        }
        else if (type == DruidRainTypes::meteorRain)
        {
            objectsAmount = 5;
            if (level == DruidParams::extra_ready)
                objectsAmount += XORRandom(10);
        ...
        else if (type == DruidRainTypes::meteorStrike)
        {
            objectsAmount = 1;
            time = 1;
        }
        else if (type == DruidRainTypes::skeletonRain)
        {
            objectsAmount = 5;
            if (level == DruidParams::extra_ready)
                objectsAmount += XORRandom(15);
        ...
        else if (type == DruidRainTypes::arrowRain)
        {
            objectsAmount = 75;
            if (level == DruidParams::extra_ready)
    ...
    bool CheckFinished()
    {
        return (type == DruidRainTypes::finished);
    }
}
...
void addRain(CBlob@ this, string type, u8 level, Vec2f pos)
{
    ...
    if (!getNet().isServer())
        return;
    if (type == "zombie_rain")
        rains.insertLast(DruidRain(this, DruidRainTypes::zombieRain, level, pos));
    else if(type == "meteor_rain")
        rains.insertLast(DruidRain(this, DruidRainTypes::meteorRain, level, pos));
    else if(type == "meteor_strike")
        rains.insertLast(DruidRain(this, DruidRainTypes::meteorStrike, level, pos));
    else if(type == "skeleton_rain")
        rains.insertLast(DruidRain(this, DruidRainTypes::skeletonRain, level, pos));
    else if(type == "arrow_rain")
        rains.insertLast(DruidRain(this, DruidRainTypes::arrowRain, level, pos));
}
...
void ManageRains( CBlob@ this )
{
    if (this.hasTag("ZombieRain"))
    {
        ...
        if (time <= 0 )
        {
            ...
            string name = DruidParams::zombieTypes[XORRandom(DruidParams::zombieTypes.length)];
