...
void DrawSpellBar(CBlob@ this)
{
	...
	u8 primarySpell;
	if (  playerPrefsInfo.hotbarAssignments_Frigate.length >= playerPrefsInfo.primaryHotkeyID )
		primarySpell = playerPrefsInfo.hotbarAssignments_Frigate[playerPrefsInfo.primaryHotkeyID];
	else
		primarySpell = 0;
	...
	for (uint i = 0; i < 15; i++)	//only 15 total spells held inside primary hotbar
	{
		...
		if ( i < 5 )		//spells 0 through 4
		{
			...
			if ( frigateMana < spellMana )
				GUI::DrawRectangle(primaryPos + Vec2f(0,64) + Vec2f(32,0)*i, primaryPos + Vec2f(32,64) + Vec2f(0, 32*(1-(frigateMana/spellMana))) + Vec2f(32,0)*i, SColor(200, 0, 0, 0));	
		...
		else if ( i < 10 )	//spells 5 through 9	
		{
			...
			if ( frigateMana < spellMana )
				GUI::DrawRectangle(primaryPos + Vec2f(0,32) + Vec2f(32,0)*(i-5), primaryPos + Vec2f(32,32) + Vec2f(0, 32*(1-(frigateMana/spellMana))) + Vec2f(32,0)*(i-5), SColor(200, 0, 0, 0));
		...
		else				//spells 10 through 14
		{
			...
			if ( frigateMana < spellMana )
				GUI::DrawRectangle(primaryPos + Vec2f(32,0)*(i-10), primaryPos + Vec2f(32,0) + Vec2f(0, 32*(1-(frigateMana/spellMana))) + Vec2f(32,0)*(i-10), SColor(200, 0, 0, 0));
	...
	GUI::DrawText(FrigateParams::spells[primarySpell].name, primaryPos + Vec2f(40,98), color_white );
	
	//primary spell mana cost
	GUI::DrawPane(primaryPos + Vec2f(0,96), primaryPos + Vec2f(32,116), color_white);
	GUI::DrawText("-" + FrigateParams::spells[primarySpell].mana, primaryPos + Vec2f(2,98), SColor(255, 158, 58, 187) );
	...
	if ( frigateMana < secondarySpellMana )
		GUI::DrawRectangle(secondaryPos, secondaryPos + Vec2f(32, 32*(1-(frigateMana/secondarySpellMana))), SColor(200, 0, 0, 0));
	...
	*/
	...
	if ( frigateMana < aux1SpellMana )
		GUI::DrawRectangle(aux1Pos, aux1Pos + Vec2f(32, 32*(1-(frigateMana/aux1SpellMana))), SColor(200, 0, 0, 0));
    ...
    Vec2f aux2Pos = Vec2f( 192.0f, getScreenHeight()-128.0f );
	...
	if ( frigateMana < aux2SpellMana )
		GUI::DrawRectangle(aux2Pos, aux2Pos + Vec2f(32, 32*(1-(frigateMana/aux2SpellMana))), SColor(200, 0, 0, 0));
