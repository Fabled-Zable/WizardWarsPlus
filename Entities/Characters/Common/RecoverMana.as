...
void onInit(CBlob@ this)
{
	...
	this.addCommandID("send mana to server");
}
...
void onTick(CBlob@ this)
{
	...
	if (getGameTime() + this.getNetworkID() % ticksPerSecond == 0)
	{
		ManaInfo@ manaInfo;
		if (!this.get( "manaInfo", @manaInfo )) 
		{ return; }
		...
		if (mana < maxMana && !this.get_bool("burnState"))
		{
			if (maxMana - mana >= adjustedManaRegenRate)
				mana += adjustedManaRegenRate;
			else
				mana = maxMana;
		}
		
		CBitStream params;
		params.write_s16(mana);
		this.SendCommandOnlyServer(this.getCommandID("send mana to server"), params);
...
{
	if (cmd == this.getCommandID("send mana to server"))
	{
		if (!isServer())
		{ return; }
		...
		s16 manaAmount;
		if (!params.saferead_s16(manaAmount))
		{ return; }
